...
welcome(Req, State) ->
	...
	% Url_Top_News1 = "http://api.contentapi.ws/news?channel=fitness&limit=6&format=short&skip=10",
	% % io:format("all news : ~p~n",[Url_all_news]),
	% {ok, "200", _, Response_Top_News} = ibrowse:send_req(Url_Top_News1,[],get,[],[]),
	% Res_Top_News = jsx:decode(list_to_binary(Response_Top_News)),
	% Params1 = proplists:get_value(<<"articles">>, Res_Top_News),
	
	Url_Top_News = string:concat("http://api.contentapi.ws/news?channel=health_news_online&limit=20&format=short&skip=", integer_to_list(SkipItems)),
	...
	Url_Top_News_Images_Limit_Skip1 = "http://api.contentapi.ws/news?channel=fitness&limit=20&skip=0&format=short",
	...
	{ok, Body} = all_news_paginated_dtl:render([{<<"news">>,ResAllNews},{"fitness_data",Params4}]),
    {Body, Req, State}	
.